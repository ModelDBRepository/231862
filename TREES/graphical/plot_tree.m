...
if islogical(ipart) && numel(ipart) == N;
    ipart = find(ipart);
end
...
if ~isempty(strfind (options,'-b'))
    ...
    if (size (color, 1) > 1) && (size (color, 2) == 1),
        HP = patch (MX, MY, MZ, 'facecolor',color','edgecolor',color');
    else
        HP = patch (MX, MY, MZ, 0);
        if (size (color, 2) == 3) && (size (color, 1) == 1),
            set (HP, 'facecolor', color,'edgecolor',color); % map color
...
if ~isempty(strfind (options, '-p'))
    ...
    if ((size (color, 1) == numel(intree.X)) && (size (color, 2) == 1)) || (size (color, 2) == numel(intree.X)) && (size (color, 1) == 1)
