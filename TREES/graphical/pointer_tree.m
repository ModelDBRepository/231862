...
switch options,
    ...
    otherwise,
        ...
        [XS YS ZS] = sphere (16);
        for ward = 1 : length (inodes),
            HP (ward) = surface (X (inodes (ward)) + llen/150*2.5 * XS + DD (1),...
                Y (inodes (ward)) + llen/150*2.5 * YS + DD (2),...
                Z (inodes (ward)) + llen/150*2.5 * ZS + DD (3));
        end
        set (HP, 'edgecolor', 'none', 'facecolor', color, 'facealpha', alpha); axis image;
end
axis equal
