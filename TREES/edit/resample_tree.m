...
%     '-w'  : waitbar
%     '-d'  : interpolates diameters (changes total surface & volume)  NOT
%     IMPLEMENTED!
...
%     '-b' do not collapse branches to multifurcations (caution, might give
%     a mess with high sr). DOES NOT PRESERVE LENGTH
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for te = 1 : length (S)
    if (~strcmp (S{te}, 'dA') && ~strcmp (S{te}, 'X') && ...
        ...
        if isvector(tree.(S{te})) && ~ischar(tree.(S{te}))   % avoids modifying string fields 
            vec = tree.(S{te});
            if numel (vec) == N
...
if isempty (strfind (options, '-v')),
    ...
    if ~isempty (icollapse),
        ...
        tree = delete_tree (tree, itodel (sub2ind (size (itodel), icollapse, 1 : length (icollapse))),doptions);
    end
end
