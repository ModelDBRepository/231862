...
% - inodes::vector: node indices, logical indexing with Mx1 vector (tree has M nodes) is also allowed {DEFAULT: last node}
...
S = fieldnames (tree);
for ward = 1 : length(S),
    if ~strcmp(S{ward},'dA') &&  ~ischar(tree.(S{ward}))  % avoids strings to be shortened
        vec = tree.(S{ward});
