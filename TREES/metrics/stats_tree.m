...
if (nargin < 4)||isempty(options),
    options = '-w -x'; % {DEFAULT: waitbar and show results}
end
    ...
    for ward = 1 : lent,
        ...
        if isempty (strfind (options, '-x')), % extras option
            ...
            if strfind (options, '-2d'),
                [khull ahull] = convhull  ( double(intrees{te}{ward}.X, intrees{te}{ward}.Y));
            else
                [khull ahull] = convhulln (double([intrees{te}{ward}.X, intrees{te}{ward}.Y, ...
                    intrees{te}{ward}.Z]));
            end
